...
list1=["Sample Name","S436A","A437G","K540E","A581G","A613S","None","A16V", "N51I", "C59R", "S108N", "I164L", "None", "K76T", "C72S", "V73V", "M74I", "N75E", "T93S", "H97Y", "F145I", "I218F", "A220S", "Q271E", "N326S", "M343L", "C350R", "G353V", "I356T", "R371I", "None", "N86Y", "Y184F", "S1034C", "N1042D", "D1246Y", "None","I258M","Y268N","None",
"P441L","F446I","G449A","N458Y","C469F","C469Y","M476I","A481V","Y493H","R515K","P527H","N537I","G538V","R539T","I543T","P553L","R561H","V568G","P574L","A578S","C580Y","D584V","R622I","A675V"]
...
for item in dict1:     
    ...
    for item2 in list1:
        ...
        #print(count)
        #print(item[0:item.find(",")])
        if item[0:item.find(",")] not in dict4:
            dict4[item[0:item.find(",")]]=item[0:item.find(",")]
        if item2=="None":
            dict4[item[0:item.find(",")]]=dict4[item[0:item.find(",")]]+","+" "
            break
        if item[item.find(",")+1+item[item.find(",")+1::].find(",")+1::] == item2:
            if dict3[item[0:item.find(",")],item[item.find(",")+1+item[item.find(",")+1::].find(",")+1::]]!="None":
                dict4[item[0:item.find(",")]]=dict4[item[0:item.find(",")]]+","+str(float(dict3[item[0:item.find(",")],item[item.find(",")+1+item[item.find(",")+1::].find(",")+1::]])*100)+"%"
                break        
        if (item[0:item.find(",")],item2) not in dict3: 
            dict3[item[0:item.find(",")],item2]=0
            #print("true")
            #print(count)
            #print(len(list1))  
                #print("true")   
            dict4[item[0:item.find(",")]]=dict4[item[0:item.find(",")]]+","+str(0)
...
for item in dict4:
    ...
    #print(dict4[])
    with open("formatnew1.csv", 'a') as t1:
        writer = csv.writer(t1)
