...
#ax.set_yticks(y2)
plt.yticks(np.arange(1,10),y2, fontsize=7)
...
for y in range(3,len(df.loc[0])):
    ...
    for x in range(len(df)):
        if type(df.loc[x][y])==str:
            if any(map(str.isdigit, df.loc[x][y])) or df.loc[x][y]=="Major" or df.loc[x][y]=="Minor":
                tempsum+=1
    wholesum+=[tempsum]
count=0
for y in (wholesum):
    count+=1
    axs3.axhline(count,xmin=(max(wholesum)-y)/max(wholesum), xmax=max(wholesum), color="green")

axs3.set_xlabel('Total number of variant calls ratios')
axs3.invert_xaxis()
