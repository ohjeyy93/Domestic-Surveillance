...
wholefiles=[]
for subdir, dirs, files in os.walk("Allreports7nor2"):
    for file in files:
...
for subdir, dirs, files in os.walk("Allreports7nor3"):
    for file in files:
        if "Domestic1Reports" not in subdir and "DS_Store" not in file:
            wholefiles+=[os.path.join(subdir, file)]
        if "Domestic1Reports" in subdir:
...
for subdir, dirs, files in os.walk("Allreports7nor4"):
    for file in files:
        if "Domestic1Reports" not in subdir and "DS_Store" not in file:
            wholefiles+=[os.path.join(subdir, file)]
        if "Domestic1Reports" in subdir:
...
wholefiles=[]
for subdir, dirs, files in os.walk("geneiousdomestic2"):
    for file in files:
        wholefiles+=[os.path.join(subdir, file)]
...
for files in wholefiles:
    with open(files,"r") as f1:
        #print(files)
        count=0
        if files!="geneiousdomestic2/.DS_Store":
            for lines in f1:
                count+=1
                ...
                if count!=1:
                    ...
                    #print(tempword1)
                    if len(tempword1[1::])==8:
                        for item in ["PfCRT","K13","PfMDR1","mitchondrial genome","DHFR","DHPS"]:
                            if item in files:
                                wholegenilist1+=[files[files.find("/")+1:files.find("_")]+","+item+","+tempword1[1]+tempword2[1::]+tempword1[-2]]
                                wholegenilist1+=[files[files.find("/")+1:files.find("_")]+","+item+","+tempword1[2]+str(int(tempword2[1::])+1)+tempword1[-1]]
...
#print(dict1)
with open("testdomcov2.csv", 'w') as t1:
    t1.write("Sample,Gene,SNP,NeST(2.0.0st),NFNeST(depthX),NFNeST(depthO),NFNeST(bbduk1),Geneious"+"\n")
    for item in dict1:
        #print(item)
