...
wholefiles=[]
for subdir, dirs, files in os.walk("Allreports8vaf1"):
    for file in files:
...
wholefiles=[]
for subdir, dirs, files in os.walk("geneiousdomesticvaf"):
    for file in files:
        wholefiles+=[os.path.join(subdir, file)]
...
for files in wholefiles:
    with open(files,"r") as f1:
        #print(files)
        count=0
        if files!="geneiousdomesticvaf/.DS_Store":
            for lines in f1:
                count+=1
...
#print(dict1)
with open("testdomvaffix1.csv", 'w') as t1:
    t1.write("Sample,Gene,SNP,NeST(2.0.0st),NFNeST(VAF),Geneious"+"\n")
    for item in dict1:
        #print(item)
